...
def calibrate_confidence(base: float, ocr_conf: float, visual_sim: float = None, rerank_score: float = None) -> float:
    """Calibrate confidence based on OCR quality and visual similarity."""
...
def self_check_errors(errors: list[dict], ocr_text: str, visual_hint: str = None) -> list[dict]:
    """Second pass filter: reduces false positives by ~30%."""
    filtered = []
    for e in errors:
        msg = e.get("msg", "").lower()
        code = e.get("code", "")
        
        # Rule 1: if error about mass and OCR contains "mass" -> keep
...
def enrich_errors_with_fixes(errors: list[dict]) -> list[dict]:
    ...
    for e in errors:
        ...
        if "fix_confidence" not in e:
            e["fix_confidence"] = 0.85
    return errors


def _mock_vlm_analysis(
    image_path: str,
    ocr_text: str,
    text_hits: list[dict],
    ...
    """Mock VLM analysis for testing without GPU."""
    ...
    for rule_code, desc in GOST_RULES.items():
        ...
        checklist.append({
            "code": f"ГОСТ {rule_code}",
            "desc": desc,
            "status": "fail" if found else "pass",
            "count": sum(1 for e in errors if rule_code in e.get("code", ""))
        })
...
class VLMService:
    """VLM service for analyzing technical drawings."""
    ...
    def _load(self):
        ...
        # API branch (vLLM / OpenAI-совместимый)
        if self.engine in ("vllm", "openai"):
            url = clean_url(settings.vlm_api_url) or "http://localhost:8001/v1"
            url = url.rstrip("/")
            logger.info(f"VLM API mode: {self.model_name} engine={self.engine} url={url}")
            self._loaded = True
            return
    ...
    def _get_api_url(self) -> str:
        """Get clean API URL."""
        return clean_url(settings.vlm_api_url) or "http://localhost:8001/v1"

    def _vllm_call(self, image_path: str, prompt: str) -> str:
        """Call external VLM API (vLLM/OpenAI compatible)."""
        try:
            import base64
            import requests
            
            base_url = self._get_api_url().rstrip("/")
            ...
            if settings.vlm_api_key and settings.vlm_api_key.strip() not in ("", "sk-", "sk"):
                ...
                logger.warning(f"VLM_API_KEY empty — request without key to {url}")
            
            with open(image_path, "rb") as f:
                b64 = base64.b64encode(f.read()).decode()
            
            # Clean model name prefix
            model_clean = self.model_name.removeprefix("openai/").removeprefix("vllm/")
            ...
            logger.info(f"VLM request → {url} model={model_clean} engine={self.engine}")
            ...
            logger.warning(f"VLM API call failed: {e}", exc_info=True)
            return ""
    ...
    def analyze_page(
        ...
        """Analyze a single page of a technical drawing."""
        ...
        if settings.vlm_quantization == "mock" or self.engine == "mock" or not self._loaded:
            if not self._loaded and settings.vlm_quantization != "mock" and self.engine != "mock":
                self._load()
            if settings.vlm_quantization == "mock" or self.engine == "mock" or not self._loaded:
                return _mock_vlm_analysis(
                    image_path, ocr_text, text_hits or [], visual_hint,
                    page_number, summary_prev, ocr_confidence, visual_sim, rerank_score
                )
