...
# Mock VLM for dev/tests; in prod loads transformers + bitsandbytes or vLLM
...
def _mock_vlm_analysis(image_path: str, ocr_text: str, text_hits: List[Dict], visual_hint: Optional[str], page_number: int, summary_prev: str="") -> Dict[str,Any]:
    ...
    # try to parse from ocr_text with regex
    ...
    if rnd.random() < 0.35:
        ...
        for i in range(n):
            ...
            errors.append(e)
    # If visual_hint exists and no error, maybe add
    ...
    # Build summary for next page context
...
class VLMService:
    ...
    def _load(self):
        ...
        try:
            ...
            logger.info(f"Loading VLM {self.model_name} quant={self.quant}")
    ...
    def analyze_page(self, image_path: str, ocr_text: str, text_hits: List[Dict]=None, visual_hint: str=None, page_number: int=1, summary_prev: str="") -> Dict[str,Any]:
        # Enforce max_context_window via truncation
        ...
        if settings.vlm_quantization=="mock" or not self._loaded:
            # Try load once
            if not self._loaded and settings.vlm_quantization!="mock":
                self._load()
            if settings.vlm_quantization=="mock" or not self._loaded:
                return _mock_vlm_analysis(image_path, ocr_text, text_hits or [], visual_hint, page_number, summary_prev)

        # Real inference path (simplified, requires actual model)
        try:
            import torch
...
"""
            ...
            # Limit generation tokens to respect context window
